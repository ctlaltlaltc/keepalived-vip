...
// getCtlRunNamespace returns the namespace in which the controller runs.
func getCtlRunNamespace() string {
...
func getPodDetails(kubeClient kubernetes.Interface) (*podInfo, error) {
	...
	myNode, err := kubeClient.CoreV1().Nodes().Get(pod.Spec.NodeName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}

	var externalIP string
	for _, address := range myNode.Status.Addresses {
		...
		if externalIP == "" && address.Type == v1.NodeInternalIP {
			externalIP = address.Address
		}
	}
...
func getClusterNodesIP(kubeClient kubernetes.Interface, nodeSelector string) (clusterNodes []string) {
...
func waitForPodRunning(kubeClient kubernetes.Interface, ns, podName string, interval, timeout time.Duration) error {
...
func waitForPodCondition(kubeClient kubernetes.Interface, ns, podName string, condition func(pod *v1.Pod) (bool, error),
