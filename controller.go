...
func (ipvsc *ipvsControllerController) getEndpoints(
	s *v1.Service, servicePort *v1.ServicePort) []service {
	epObj, epExists, err := ipvsc.endpoints.Get(s)
	if err != nil {
		glog.Warningf("unexpected error getting service endpoints: %v", err)
		return []service{}
	}
	if !epExists {
		glog.Warningf("endpoint %v not found", s)
		return []service{}
	}

	var endpoints []service
	ep := epObj.(*v1.Endpoints)
...
func (ipvsc *ipvsControllerController) getServices(cfgMap *v1.ConfigMap) []vip {
	svcs := []vip{}
	...
	for externalIP, nsSvcLvs := range cfgMap.Data {
		...
		svcObj, svcExists, err := ipvsc.services.GetByKey(nsSvc)
		if err != nil {
...
func (ipvsc *ipvsControllerController) getConfigMap(ns, name string) (*v1.ConfigMap, error) {
...
func newIPVSController(kubeClient kubernetes.Interface, namespace string, useUnicast bool, configMapName string, vrid int, vrrpVersion int) *ipvsControllerController {
	...
	pod, err := kubeClient.CoreV1().Pods(podInfo.PodNamespace).Get(podInfo.PodName, metav1.GetOptions{})
	if err != nil {
	...
	informer := informers.NewSharedInformerFactoryWithOptions(kubeClient, resyncPeriod,
	...
	ipvsc.serviceInformer = informer.Core().V1().Services().Informer()
	ipvsc.serviceInformer.AddEventHandler(eventHandlers)
	ipvsc.services = ipvsc.serviceInformer.GetStore()

	ipvsc.endpointInformer = informer.Core().V1().Endpoints().Informer()
	ipvsc.endpointInformer.AddEventHandler(eventHandlers)
	ipvsc.endpoints = ipvsc.endpointInformer.GetStore()
